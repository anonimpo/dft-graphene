...
def plot_ecut_convergence(stacking, layers):
    """
    Plot the energy vs. cutoff energy convergence data.
    ...
    if stacking == "1L":
        data_file = f"1L/results/ecut_vs_energy.dat"
        title_prefix = "Monolayer"
    else:
        data_file = f"{stacking}/{layers}L/results/ecut_vs_energy.dat"
    ...
    try:
        ...
        # Plot energy difference vs. ecut (convergence)
        ...
        if stacking == "1L":
            output_file = "./1L/results/1L_ecut_convergence.pdf"
        else:
            output_file = f"./{stacking}/{layers}L/results/{stacking}_{layers}L_ecut_convergence.pdf"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        print("Usage: python plot_ecut.py <stacking_type> <num_layers>")
        print("Examples:")
        print("  python plot_ecut.py          # Plot monolayer (default)")
        print("  python plot_ecut.py 1L 1     # Plot monolayer")
        print("  python plot_ecut.py AA 2     # Plot AA-stacked bilayer")
        sys.exit(1)

    plot_ecut_convergence(stacking, layers)
